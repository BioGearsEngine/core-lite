...
void SaturationCalculator::CalculateSaturation(SELiquidCompartment& cmpt)
{
  ...
  double bufferBase_mM = m_data.GetState() > EngineState::InitialStabilization ? CalculateStrongIonDifference(cmpt) : 45.0;
  if (cmpt.GetName() == "Aorta") {
    m_data.GetDataTrack().Probe("Aorta-SID", bufferBase_mM);
  }
  if (cmpt.GetName() == "VenaCava") {
    m_data.GetDataTrack().Probe("VenaCava-SID", bufferBase_mM);
  }
  ...
  auto hp_alt = [&](double Hp_mM) { return b2 * Hp_mM * Hp_mM + b1 * Hp_mM + b0; };
  auto hp_alt_dir = [&](double Hp_mM) { return 2.0 * b2 * Hp_mM + b1; };

  //HpPlasma_mM = NewtonRaphsonSolver(hp_function, hp_derivative, HpPlasma_mM, 1.0e-6, 20);
  HpPlasma_mM = NewtonRaphsonSolver(hp_alt, hp_alt_dir, HpPlasma_mM, 1.0e-6, 20);
...
double SaturationCalculator::CalculateStrongIonDifference(SELiquidCompartment& cmpt)
{
  ...
  return sodium_mM + potassium_mM - chloride_mM - lactate_mM;
}
